...
def parse_args():
    ...
    global args, emu_addr, HELLO, HOST_IPV4, WAITING_NS, TIMEOUT_NS
...
def create_packets_from_file():
    ...
    with open(request_file, 'rb') as file:
        while True:
            ...
            packet = create_packet(
                PacketType.DATA, seq_no, chunk, len(chunk), requester)
    ...
    seq_no = next(sequence)
    end_packet = create_packet(PacketType.END, seq_no, bytes(), 0, requester)
